function applyGaussianFilter(array) {
          const image = new Image([array]);
          const filteredImage = image.gaussianFilter({ sigma: 1 });

          // Access the filtered data directly
          const filteredData = filteredImage.data[0];

          // Create a new Int16Array with the filtered data
          const resultArray = new Int16Array(filteredData);

          return resultArray;
        }

        function applyGaussianFilterToSlices(slices) {
          return slices.map((slice) => slice.map(applyGaussianFilter));
        }

        // Apply Gaussian filter to all slices
        this.volume = applyGaussianFilterToSlices(this.volume);
