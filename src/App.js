...
const styles = (theme) => ({
  ...
  appBar: {
    ...
    backgroundColor: "#110a07", // dark brown background for a formal app bar
  },
  ...
  drawer: {
    ...
    flexShrink: 0,
    whiteSpace: "nowrap",
    backgroundColor: "#1f1f1f", // Dark gray background for the Drawer
  },
  // Add this to target the Paper component inside the Drawer
  paper: {
    backgroundColor: "#1f1f1f", // Or any color you wish to set
  },
...
class App extends PureComponent {
  ...
  mprBuildVolume = () => {
    ...
    const files = this.dicomViewersRefs[0].files.filter(
      (a) => a.series.echoNumber === echoNumber
    );

    // Update files and echo number if necessary
    ...
    const width = files[sliceIndex].image.columns; // Image width
    const height = files[sliceIndex].image.rows; // Image height
    ...
    if (files.length === this.mprData.zDim) {
      ...
      if (interpolationMethod === "no") {
        // Fill missing planes without interpolation (simple duplicate)
        ...
        // Interpolate planes between original planes using bilinear interpolation for non-edge pixels and cubic interpolation for edge pixels
        for (let i = 0; i < intervals.length - 1; i++) {
          const step = intervals[i + 1] - intervals[i];
          for (let j = intervals[i + 1] - 1; j > intervals[i]; j--) {
            ...
            for (let k = 0; k < length; k++) {
              ...
              if (isEdgePixel(row, col, width, height)) {
                ...
                const p0 =
                  this.volume[intervals[i]][k] * (1 - w) +
                  this.volume[intervals[i + 1]][k] * w;
                p[k] = p0;
              }
            }
    ...
    // Check if a pixel is an edge pixel based on its 2D coordinates (row and column)
    function isEdgePixel(row, col, width, height) {
