...
class DicomViewer extends React.Component {
  ...
  //Handles mouse scroll events to navigate through frames of open DICOM files
  ...
  //Handles the progress of loading a DICOM file from a URL
  onOpenUrl = (e) => {
    const eventData = e.detail;
    this.setState({ progress: eventData.percentComplete });
  };
  //Initiates the process of opening a DICOM file from a URL and displays a dialog
  showOpenUrlDlg = (url) => {
  ...
  //Hides the open URL dialog
  hideOpenUrlDlg = () => {
    this.setState({ visibleOpenUrlDlg: false, progress: null });
  };

  //Adds a measurement object to the measurements array
  measurementSave = (measure) => {
    this.measurements.push(measure);
  };

  //Clears all measurements from the measurements array
  measurementClear = () => {
    this.measurements.splice(0, this.measurements.length);
  };

  //Removes a measurement at a specific index from the measurements array
  measurementRemove = (index) => {
    //console.log('this.measurements: ', this.measurements)
    this.measurements.splice(index, 1);
  };
  //Retrieves the Transfer Syntax of the loaded DICOM image
  getTransferSyntax = () => {
    const value = this.image.data.string("x00020010");
    return value + " [" + uids[value] + "]";
  };
  //Retrieves the SOP Class UID (Unique Identifier) of the loaded DICOM image
  getSopClass = () => {
    const value = this.image.data.string("x00080016");
    return value + " [" + uids[value] + "]";
  };
  //Retrieves the SOP Instance UID of the loaded DICOM image
  getSopInstanceUID = () => {
    const value = this.image.data.string("x00080018");
    return value;
  };
  //Retrieves the pixel representation (signed or unsigned) of the loaded DICOM image
  getPixelRepresentation = () => {
    const value = this.image.data.uint16("x00280103");
    if (value === undefined) return;
    return value + (value === 0 ? " (unsigned)" : " (signed)");
  };
  //Retrieves the planar configuration (pixel or plane) of the loaded DICOM image
  getPlanarConfiguration = () => {
    const value = this.image.data.uint16("x00280006");
    if (value === undefined) return;
    return value + (value === 0 ? " (pixel)" : " (plane)");
  };
  //Retrieves the DOM elements with the class "cornerstone-enabled-image" (likely the DICOM viewer elements)
  getDicomViewerElement = () => {
    return document.getElementsByClassName("cornerstone-enabled-image");
  };
  //Invoked when the DICOM image has been successfully loaded
  onImageLoaded = (e) => {
  ...
  onImageRendered = (e) => {
    ...
    this.zoom = Math.round(viewport.scale.toFixed(1) * 100);

    // Check if the element with the specified ID exists before updating content
    const topLeftElement = document.getElementById(
      `mrtopleft-${this.props.index}`
    );
    if (topLeftElement) {
      topLeftElement.textContent = this.mprIsOrthogonalView()
    ...
    if (bottomLeftElement) {
      bottomLeftElement.textContent = `WW/WC: ${Math.round(
    ...
    if (bottomRightElement) {
      bottomRightElement.textContent = `Zoom: ${this.zoom}%`;
    }

    const topCenterElement = document.getElementById(
      `mrtopcenter-${this.props.index}`
    );
    if (topCenterElement) {
      topCenterElement.textContent = ``;
    }

    const bottomCenterElement = document.getElementById(
      `mrbottomcenter-${this.props.index}`
    );
    if (bottomCenterElement) {
      bottomCenterElement.textContent = ``;
    }

    const leftCenterElement = document.getElementById(
      `mrleftcenter-${this.props.index}`
    );
    if (leftCenterElement) {
      leftCenterElement.textContent = ``;
    }

    const rightCenterElement = document.getElementById(
      `mrrightcenter-${this.props.index}`
    );
    if (rightCenterElement) {
      rightCenterElement.textContent = ``;
    }

    if (this.mprPlane === "sagittal") {
      if (topCenterElement) topCenterElement.textContent = `S`;
      if (bottomCenterElement) bottomCenterElement.textContent = `I`;
      if (leftCenterElement) leftCenterElement.textContent = `A`;
      if (rightCenterElement) rightCenterElement.textContent = `P`;
    } else if (this.mprPlane === "axial") {
      if (topCenterElement) topCenterElement.textContent = `A`;
      if (bottomCenterElement) bottomCenterElement.textContent = `P`;
      if (leftCenterElement) leftCenterElement.textContent = `R`;
      if (rightCenterElement) rightCenterElement.textContent = `L`;
    ...
    // Slice Location Check for MPR
    if (this.mpr.isSliceLocation) {
      this.mpr.isSliceLocation = false;
      this.mprSliceLocationDraw();
    }

    // DICOM Cine Player Frame Update
    if (
      this.isDicom &&
      this.state.visibleCinePlayer &&
      this.numberOfFrames > 1
    ) {
      const frameLabelElement = document.getElementById(
        `frameLabel-${this.props.index}`
      );
      if (frameLabelElement) {
        frameLabelElement.textContent = `${this.state.frame} / ${this.numberOfFrames}`;
      }
  ...
  //This function is triggered when a measurement is modified
  onMeasurementModified = (e) => {
    //console.log('cornerstonetoolsmeasurementmodified: ', e.detail.measurementData)
  };
  //This function is called when a new measurement is added. It checks if the tool used is the "Angle" tool
  ...
  //This function is invoked when a measurement is completed.
  //It creates a measurement object and saves it. If the tool is "FreehandRoi
  ...
  //The function provides a comprehensive setup for displaying DICOM images
  ...
  //This function is intended to load and display a DICOM image from a canvas
  ...
  //This function is intended to load and display a DICOM image from a custom object
  ...
  //The function is designed to load and display DICOM images based on various sources such as local files, URLs, or file system items
  ...
  //The function is designed to enable a set of predefined tools for interaction with a DICOM image displayed in the web app
  ...
  runTool = (toolName, opt) => {
    ...
    switch (toolName) {
      case "setfiles": {
        ...
        // console.log(
        //   `Performance: runTool time setfiles- ${
        //     this.t1 - this.t0
        //   } milliseconds`
        // );
      ...
      case "openimage": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time openimage- ${
        //     this.t1 - this.t0
        //   } milliseconds`
        // );
        break;
      ...
      case "openLocalFs": {
        ...
        this.t1 = performance.now();
        console.log(
          `Performance: runTool time openLocalFs- ${
            this.t1 - this.t0
          } milliseconds`
        );
        break;
      ...
      case "clear": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time clear- ${this.t1 - this.t0} milliseconds`
        // );
        break;
      }
      case "notool": {
      ...
      case "Zoom": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time Zoom- ${this.t1 - this.t0} milliseconds`
        // );
        break;
      }
      case "Length": {
      ...
      case "Magnify": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time Magnify- ${this.t1 - this.t0} milliseconds`
        // );
        break;
      }
      case "FreehandRoi": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time FreehandRoi- ${
        //     this.t1 - this.t0
        //   } milliseconds`
        // );
        break;
      ...
      case "Invert": {
        ...
        this.t1 = performance.now();
        console.log(
          `Performance: runTool time Invert- ${this.t1 - this.t0} milliseconds`
        );
        break;
      }
      case "saveas": {
      ...
      case "cine": {
        ...
        this.t1 = performance.now();
        console.log(
          `Performance: runTool time cine- ${this.t1 - this.t0} milliseconds`
        );
        break;
      }
      case "reset": {
    ...
    // this.t1 = performance.now();
    // console.log(
    //   `Performance: runTool time - ${this.t1 - this.t0} milliseconds`
    // );
  };

  //The cropCanvas function is a utility function used for cropping an existing HTML canvas
  ...
  //The changeTool function is responsible for dynamically changing the state (active or passive) of specific tools
  ...
  //The runCinePlayer appears to control the behavior of a cine player for displaying a sequence of DICOM frames
  runCinePlayer = (cmdName) => {
    ...
    switch (cmdName) {
      case "firstframe": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time firstframe- ${
        //     this.t1 - this.t0
        //   } milliseconds`
        // );
        break;
      ...
      case "previousframe": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time previousframe- ${
        //     this.t1 - this.t0
        //   } milliseconds`
        // );
        break;
      ...
      case "play": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time play- ${this.t1 - this.t0} milliseconds`
        // );
        break;
      }
      case "pause": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time pause- ${this.t1 - this.t0} milliseconds`
        // );
        break;
      }
      case "nextframe": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time nextframe- ${
        //     this.t1 - this.t0
        //   } milliseconds`
        // );
        break;
      ...
      case "lastframe": {
        ...
        // this.t1 = performance.now();
        // console.log(
        //   `Performance: runTool time lastframe- ${
        //     this.t1 - this.t0
        //   } milliseconds`
        // );
        break;
  ...
  //The reset function is designed to reset the viewport settings of a DICOM image element to default values
  reset = () => {
    const element = this.dicomImage;
  ...
  // -------------------------------------------------------------------------------------------- MPR
  //#region MPR
  //The mprPlanePosition function is designed to determine the orientation or plane of a DICOM image series in MPR.
  mprPlanePosition = () => {
    try {
  ...
  //Renders the YZ plane in the Multi-Planar Reconstruction
  ...
  //Builds the YZ plane for the given X-coordinate
  mprBuildYZPlane = (x) => {
  ...
  //Renders the XZ plane in the Multi-Planar Reconstruction
  ...
  //Builds the XZ plane for the given Y-coordinate
  mprBuildXZPlane = (y) => {
  ...
  //Checks if the current MPR view is orthogonal (e.g., axial, sagittal, coronal)
  mprIsOrthogonalView = () => {
    return (
      this.mprPlane !== "" &&
  ...
  //Computes and sets reference lines for the MPR view
  ...
  //Similar to mprReferenceLines, but used in a different context (from second view to third view or vice versa)
  ...
  //Draws the slice location on the canvas
  ...
  //
  //Builds reference lines based on the orientation and geometry of two images (this.image and srcImage)
  ...
  //Constructs a reference line between two images in 3D space
  ...
  //Transforms a 3D patient point (p) to its corresponding 2D pixel coordinates in the destination image
  ...
  //Builds a plane in the pixel coordinates of the destination image based on the geometry of the source image
  ...
  //Draws the reference lines and plane on the canvas
  referenceLinesDraw = () => {
  ...
  //#endregion
  //Sets the dicomImage property to the provided element (el), presumably a reference to a DICOM image element in the DOM
  dicomImageRef = (el) => {
    this.dicomImage = el;
  };
  //A placeholder function that is currently commented out. It might be intended for handling click events on the DICOM image
  onImageClick = () => {
    //console.log('onImageClick: ')
  };
  //Determines whether the current DICOM image is a localizer image
  isLocalizer = () => {
    return isLocalizer(this.image);
  };
  //Finds the index of the first DICOM slice with a given Image Position Patient (IPP) value along a specified axis (ippPos)
...
//This function maps action creators to props, making these action creators available to the component
...
//The connect function from react-redux is used to connect the DicomViewer component to the Redux store
