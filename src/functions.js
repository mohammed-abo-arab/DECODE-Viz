...
export function getDicomNearestAxis(image) {
  const ipp = image.data
    .string("x00200032")
    .split("\\")
    .map((v) => parseFloat(v)); // Image Position Patient - x, y, z of top hand corner
  const xabs = Math.abs(ipp[0]);
  const yabs = Math.abs(ipp[1]);
  const zabs = Math.abs(ipp[2]);
  let axes = Vector.zero;
  if (xabs >= yabs && xabs >= zabs) axes.x = ipp[0] > 0.0 ? 1.0 : -1.0;
  else if (yabs >= zabs) axes.y = ipp[1] > 0.0 ? 1.0 : -1.0;
  else axes.z = ipp[2] > 0.0 ? 1.0 : -1.0;
  return axes;
}

export function getDicomImageXOnRows(image) {
  const iop = image.data
    .string("x00200037")
    .split("\\")
    .map((v) => parseFloat(v));
...
export function isFsFileImage(fsItem) {
  ...
  return (
    fsItem.type.toLowerCase() === "jpeg" || fsItem.type.toLowerCase() === "png"
  );
}
